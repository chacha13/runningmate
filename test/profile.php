...
$result_user = mysql_query("SELECT * FROM users WHERE id = '$uid'", $connection);
$myrow_user = mysql_fetch_array($result_user);
$full_name = $myrow_user['firstname'].' '.$myrow_user['lastname'];
...
<?
	$result_pic2 = mysql_query("SELECT * FROM photos WHERE user_id = '$uid'", $connection);
	$myrow_pic2 = mysql_fetch_array($result_pic2);
	
	$result_feeds2 = mysql_query("SELECT * FROM message_feeds WHERE user_id = '$uid' ORDER BY created_at DESC", $connection);
...
$(document).ready(function(){
	...
	var datevar      = $('#datevar').attr('value');
		...
		$.ajax({
			...
			success: function(){
				$("ul#wall").prepend("<li style='display:none'><img src='uploads/<? echo $myrow_pic2[2]?>' align='left' hspace='15px' width='50' height='40' style='margin-left: 5px;' /><a href='profile.php?uid=<? echo $uid?>'><? echo $full_name?></a> "+message_wall+"<br />"+"<small>"+datevar+"<a href='#' class='like' id='<? echo $myrow_feeds2[0] ?>'>"+"Like"+"</a></small>"+"</li>");
...
<form name="search" action="search.php" method="get">
	<div id="search_box">
	      Search Runners: <input id="q" name="q" type="text" class="search">
...
<ul id="wall">
<?
$result_feeds = mysql_query("SELECT * FROM message_feeds WHERE user_id = '$uid' ORDER BY created_at DESC", $connection);
$myrow_feeds = mysql_fetch_array($result_feeds);

if($myrow_feeds !=""){
	do {
	$result_likes = mysql_query("SELECT * FROM message_feeds_like WHERE post_id = '$myrow_feeds[0]' AND user_id = '$uid' AND like_id = 1", $connection);
	...
	$result_pic = mysql_query("SELECT * FROM photos WHERE user_id = '$uid'", $connection);
	$myrow_pic = mysql_fetch_array($result_pic);
	
		echo '<li id="list'.$myrow_feeds[0].'">';
		if($myrow_pic[2] != '') {
		...
		echo $myrow_feeds[2];
                ...
                if($myrow_feeds[0] == $myrow_likes[0] && $myrow_likes[1] == $uid && $myrow_likes[2] == 1) {
                	echo '';
                	echo 'Unlike';
                	echo '</a>';
                }
		...
		<?  	
		if($_SESSION['user_id'] == $myrow_feeds['user_id']) {
			echo "<a href=\"#\" id=\"$myrow_feeds[0]\" class=\"delete\">";
	                echo 'Delete </a>';
	                //echo '</small>';
	               }
	               else {
...
<a href="http://tangkilikan.com.ph" target="_blank">Tangkilikan.com.ph</a> | <a href="http://runningmate.ph" target="_blank">Runningmate.ph</a>
     ...
     <div id="futuro">
     	...
     	<a href="javascript:void(0)" onclick="javascript:chatWith('shila')">Shila</a>
